...
MODELS = ROOT / "models"
PORT = int(os.environ.get("PORT", 8081))    # INT8 dashboard: separate port from the FP16 app (8080)
APP_TAG = os.environ.get("APP_TAG", "int8")  # namespace for shm frames + gpu telemetry (detached from FP16)
...
def main():
    ...
    print(f"[INT8] models: det={DET_MODEL}@{DET_DEV}  emb={EMB_MODEL}@{EMB_DEV}  "
          f"(multiprocessing: 1 process/stream + 1 shared re-id gallery = cross-cam)", flush=True)
    print(f"INT8 stream dashboard on http://0.0.0.0:{PORT}", flush=True)
