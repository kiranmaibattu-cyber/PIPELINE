...
BASE_TAG="${INTEL_TRAFFIC_V16_BASE_TAG:-localhost/apexfabric-intel-traffic-runtime-base:intel-285h-2026.09.18-v2}"
IMAGE_VERSION="${APEXFABRIC_IMAGE_VERSION:-2026.09.22-v16}"
IMAGE_TAG="localhost/traffic-pilot-runtime:intel-285h-${IMAGE_VERSION}"

if [[ "${BUILD_TRAFFIC_V16_BASE:-0}" == "1" ]]; then
...
if ! podman image inspect "$BASE_TAG" >/dev/null 2>&1; then
  ...
  echo "Load the Intel base or run with BUILD_TRAFFIC_V16_BASE=1." >&2
  exit 1
fi
...
podman build --platform linux/amd64 \
  ...
  --build-arg "IMAGE_VERSION=${IMAGE_VERSION}" \
    -f docker/Dockerfile.traffic-v16 \
  -t "$IMAGE_TAG" .
