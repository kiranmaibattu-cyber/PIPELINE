"""Edge load-test application for the MTMC OpenVINO pipeline.
...
from MTMC.ov_backends import OVReidEmbedder, OVDetector  # noqa: E402

MODELS = ROOT / "models"
PORT = int(os.environ.get("PORT", 8080))
APP_TAG = os.environ.get("APP_TAG", "sa")   # namespace for shm frames + gpu telemetry so a 2nd
#                                             # instance (e.g. INT8 on another port) doesn't collide
DET_DEV = os.environ.get("DET_DEV", "GPU")
EMB_DEV = os.environ.get("EMB_DEV", "NPU")
DET_MODEL = os.environ.get("DET_MODEL", "yolo11s.xml")   # bigger detector: recovers seated/occluded people
...
EMB_MODEL = os.environ.get("EMB_MODEL", "transreid_ssl_fp16.xml")
FACE_MODEL = os.environ.get("FACE_MODEL", "adaface_ir101_fp16.xml")
GAIT_MODEL = os.environ.get("GAIT_MODEL", "gaitbase_fp16.xml")
...
DESIRED_THR = None       # live re-id threshold; new stream processes inherit it (fork)
...
def run_stream(sid, source, camera, do_face, do_gait, mdict, crop_q, stop_ev, desired_thr, req_q, resp_q):
    ...
    try:
        ...
        det = OVDetector(str(MODELS / DET_MODEL), DET_CONF, 0.5, {0}, device=DET_DEV)
        tracker = IoUTracker()
        emb = OVReidEmbedder(str(MODELS / EMB_MODEL), device=EMB_DEV)
        face = gait = None
        if do_face:
            from MTMC.face_embedder import AdaFaceEmbedder
            face = AdaFaceEmbedder(backend="openvino", ov_xml=str(MODELS / FACE_MODEL), ov_device=FACE_DEV)
        if do_gait:
            from MTMC.gait_embedder import OnlineGaitEmbedder
            gait = OnlineGaitEmbedder(backend="openvino", ov_xml=str(MODELS / GAIT_MODEL), ov_device=GAIT_DEV,
                                      ov_seg_xml=str(MODELS / SEG_MODEL), ov_seg_device=SEG_DEV)
        ...
        while not stop_ev.is_set():
            ...
            if embs_e is not None and len(embs_e) == len(eidx) and eidx:
                ...
                try:
                    ...
                    egids, edists = [None] * len(eidx), [99.0] * len(eidx)
                for j, i in enumerate(eidx):
    ...
    def __init__(self, sid, source, camera, do_face, do_gait):
        ...
        self.proc = multiprocessing.Process(
            ...
            args=(sid, source, camera, do_face, do_gait, self.mdict, CROP_Q, self.stop_ev,
                  DESIRED_THR, REID_REQ_Q, self.resp_q))
...
def main():
    ...
    print(f"models: det={DET_MODEL}@{DET_DEV}  emb={EMB_MODEL}@{EMB_DEV}  "
          f"(multiprocessing: 1 process/stream + 1 shared re-id gallery = cross-cam)", flush=True)
    print(f"stream load-test dashboard on http://0.0.0.0:{PORT}", flush=True)
